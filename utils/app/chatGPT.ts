...
export const gradeSenseUsingChatGPT = async (
  ...
  for (const chat_gpt_role of answer.chat_gpt_roles) {
    const score = await getSenseScore(problem, answer, systemPrompt, chat_gpt_role, lastUserMessage,temperature)
    // 逐次平均
...
async function getSenseScore(
  problem: any, answer: any, systemPrompt: string,
  chat_gpt_role: string, lastUserMessage: any,temperature: Temperature,
):Promise<Score> {
    let chatGPTResponse: ChatGPTResponse | undefined;
    try {
      const model:MODEL = answer?.model || 'gpt-3.5-turbo-0613'
      chatGPTResponse = await senseChatGPT(model,systemPrompt,chat_gpt_role,answer.scoring_criteria,
        problem.score,problem.content,lastUserMessage.content,temperature)
    } catch (error: any) {
...
async function senseChatGPT(model:MODEL,systemPrompt: string, chat_gpt_role:string, 
  scoring_criteria:string, problem_score: string,
  content:string,lastUserMessage_content: string,
  temperature:Temperature):Promise<ChatGPTResponse>{
  let chatGPTResponse: ChatGPTResponse | undefined;
...
async function emulateChatGPT(
  model:MODEL, systemPrompt: string,
  messages: Message[], input: string, temperature: Temperature): Promise<ChatGPTResponse>{
...
async function testPrompt(
  model:MODEL, input:any, correctAnswer:any,
  system_prompt:string,messages:Message[], temperature: Temperature,
  ):Promise<{success:boolean,correctAnswer:any,res:ChatGPTResponse | undefined} > {
    let result = {
      correctAnswer,
      success: false,
...
export const gradedMultipleCaseUsingChatGPT = async (
  ...
  for (let i = 0; i < answer.inputs.length; i++) {
    ...
    promises.push(testPrompt(model, answer.inputs[i], correctAnswer, systemPrompt, messages, temperature))
  }
  ...
  results.forEach(result => {
    if(result.success){
      ...
      if (result.res){
        promiseReason = MakeWrongReasonCompared(result.res.utterances[0].content,result.correctAnswer)
      }
    }
  });
